...
INSERT INTO faq (faq_id, category, question, answer, ord) VALUES ('Q-02', 'Начало работы', 'Нужен ли интернет или сервер?', 'Нет. Приложение работает локально в браузере, данные хранятся в localStorage. Если поднят backend (Java + Python), состояние дополнительно синхронизируется через /api/state — индикатор в шапке показывает статус.', 1);
INSERT INTO faq (faq_id, category, question, answer, ord) VALUES ('Q-03', 'Начало работы', 'Как быстро найти нужный раздел или действие?', 'Нажмите Ctrl/⌘+K — откроется командная палитра с поиском по всем разделам и действиям (экспорт, смена темы, сброс и т.д.).', 2);
INSERT INTO faq (faq_id, category, question, answer, ord) VALUES ('Q-04', 'Планирование', 'Как рассчитывается ёмкость спринта?', 'Из состава «Команды» и её доступности, отсутствий, числа рабочих дней спринта (с учётом «Календаря» праздников) и focus factor из «Параметров».', 3);
INSERT INTO faq (faq_id, category, question, answer, ord) VALUES ('Q-05', 'Планирование', 'Что такое focus factor?', 'Доля рабочего времени, реально уходящая на задачи спринта (обычно 0.7–0.8). Остаток — митинги, поддержка, переключения между задачами. Задаётся в «Параметрах».', 4);
INSERT INTO faq (faq_id, category, question, answer, ord) VALUES ('Q-06', 'Планирование', 'Чем отличаются RICE и MoSCoW?', 'RICE даёт числовой балл (Reach × Impact × Confidence / Effort) для ранжирования фич по приоритету. MoSCoW распределяет фичи по категориям Must / Should / Could / Won''t. Их удобно использовать вместе: RICE — что важнее внутри списка, MoSCoW — что вообще войдёт в релиз.', 5);
INSERT INTO faq (faq_id, category, question, answer, ord) VALUES ('Q-07', 'Данные и доступ', 'Где хранятся мои данные?', 'Локально в браузере (localStorage) и, если настроен сервер, в базе PostgreSQL через /api/state. Для резервной копии выгружайте JSON или Excel.', 6);
INSERT INTO faq (faq_id, category, question, answer, ord) VALUES ('Q-08', 'Данные и доступ', 'Как перенести данные на другой компьютер?', 'На текущем компьютере нажмите «JSON» (экспорт), на другом — «Импорт» и выберите сохранённый файл. Импорт доступен администратору.', 7);
INSERT INTO faq (faq_id, category, question, answer, ord) VALUES ('Q-09', 'Данные и доступ', 'Что делает кнопка «Сброс»?', 'Возвращает данные к исходному демонстрационному набору. Действие необратимо и доступно только администратору — перед сбросом сделайте экспорт JSON.', 8);
INSERT INTO faq (faq_id, category, question, answer, ord) VALUES ('Q-10', 'Данные и доступ', 'Чем отличаются режимы Просмотр, Редактор и Администратор?', 'Просмотр — только чтение, редактирование недоступно. Редактор правит данные во всех разделах, кроме «Параметров», «Сброса» и «Импорта». Администратор — полный доступ, включая параметры и управление кодами доступа.', 9);
INSERT INTO faq (faq_id, category, question, answer, ord) VALUES ('Q-11', 'Данные и доступ', 'Как сменить режим доступа?', 'Кнопка режима в шапке (👁/✏️/🛡). Переход в «Редактор» или «Администратор» — по коду, который задаёт администратор в панели «Коды доступа».', 10);
